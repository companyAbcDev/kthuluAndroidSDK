...
suspend fun getNodeHomeAsync(network: String, toNetwork: String, token_address: String): JSONObject = withContext(Dispatchers.IO) {
    networkSettings(network)
    ...
    try {
        ...
        if (customNhid != BigInteger.ZERO) {
            val getTokNetworkFunction = Function("getToNetwork", listOf(Address(token_address), Uint256(toNetworkHex)), emptyList())
            val encodedGetTokNetworkFunction = FunctionEncoder.encode(getTokNetworkFunction)
            val getTokNetworkResponse = web3j.ethCall(
                Transaction.createEthCallTransaction(null, bridgeConfigContractAddress, encodedGetTokNetworkFunction),
                DefaultBlockParameterName.LATEST
            ).send()
